  ...
  async function loadAll(){
    ...
    try {
      ...
      if (!sRes.error && sRes.data && sRes.data.value) {
        settings = Object.assign(defaultSettings(), sRes.data.value);
      }
...
async function deleteDm(e){
  ...
  async function drawContact(ctx){
    ...
    var safeTextW = Math.max(60, b.w - 170);
    ...
    var nameText = c.name || '';
    var nameMaxW = Math.floor(safeTextW * 0.55);
    ...
    fitText(ctx, c.title || '', textX + nameW + 16, y + nameSize, safeTextW - nameW - 16, titleSize + 4);
    ...
    if (photo) {
      ...
      ctx.clip();
      drawPortraitCrop(ctx, photo, b.photoX, b.photoY, b.photoSize, b.photoSize);
      ctx.restore();
    }

    var qr = await loadImage(qrSrc);
    if (qr) {
      ctx.fillStyle = '#fff';
      ctx.fillRect(b.qrX - 4, b.qrY - 4, b.qrSize + 8, b.qrSize + 8);
      drawSquareContain(ctx, qr, b.qrX, b.qrY, b.qrSize, b.qrSize);
    }
  }
  ...
  function drawPortraitCrop(ctx, img, x, y, w, h){
    ...
    var sx = (iw - sw) / 2;
    var sy = Math.max(0, Math.min(ih - sh, ih * 0.10));
